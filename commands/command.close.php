...
class COMMAND_CLOSE
{
	...
	function COMMAND_CLOSE(&$sys_message)
	{
	...
	{
		$this->INTERPRETER->ADD_ACTION($this->NAME,"CLOSE", "CLOSE", "", $this->REQUIRED_FLAG);
	...
	function CLOSE($target)
	{
		...
		if($target != "")
		{
			if(trim($cmd[0]) == "DOOR")
			{
				...
				if($direction == "0")
				{
					...
					if($this->EXITS->EXIT_EXISTS($zone, $room, $direction))
					{
						...
						{
							if($this->EXITS->IS_DOOR_OPENED($zone, $room, $direction))
							{
								if($this->EXITS->IS_DOOR_LOCKED($zone, $room, $direction))
								{
									$this->COMMUNICATION->SEND_TO_CHAR("That door is locked.\n");
								}else{
									$this->EXITS->CLOSE_DOOR($zone, $room, $direction);
									$this->EXITS->CLOSE_DOOR($this->EXITS->GET_EXIT_DEST_ZONE($zone, $room, $direction), $this->EXITS->GET_EXIT_DEST_ROOM($zone, $room, $direction), $this->MOVEMENT->GET_REVERSE_DIRECTION($direction));
									$this->COMMUNICATION->SEND_TO_CHAR("The door is now closed.\n");
									$this->COMMUNICATION->SEND_TO_ROOM_EXCEPT($zone, $room, "\n".$firstname." ".$lastname." closes the door.\n");
									$this->COMMUNICATION->SEND_TO_ROOM_EXCEPT($zone, $room, "> ");
									$this->COMMUNICATION->SEND_TO_ROOM($this->EXITS->GET_EXIT_DEST_ZONE($zone, $room, $direction), $this->EXITS->GET_EXIT_DEST_ROOM($zone, $room, $direction), "\nThe door closes from the other side.\n");
									$this->COMMUNICATION->SEND_TO_ROOM($this->EXITS->GET_EXIT_DEST_ZONE($zone, $room, $direction), $this->EXITS->GET_EXIT_DEST_ROOM($zone, $room, $direction), "> ");
								}
							}else{
								$this->COMMUNICATION->SEND_TO_CHAR("The door is already closed.\n");
							}
						}else{
			...
			$this->COMMUNICATION->SEND_TO_CHAR("What do you want to close?\n");
		}
	}
}
?>