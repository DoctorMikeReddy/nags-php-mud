...
class COMMAND_UP
{
	var $NAME = "COMMAND_UP";
     ...
     function COMMAND_UP(&$sys_message)
     {
     ...
     {
          $this->INTERPRETER->ADD_ACTION($this->NAME,"UP", "UP", "", $this->REQUIRED_FLAG);
          $this->PFLAGS->ADD_FLAG($this->REQUIRED_FLAG);
          $this->INTERPRETER->ADD_ALIAS("U", "UP");
          return True;
     }
     function UP()
     {
		$socket = $this->SYSTEM->CURRENT_SOCKET;
		$this->MOVEMENT->GO_THROUGH_EXIT("UP");
     }  
}
?>